...
function getCookie(cname) { // sets function to get cookie 
    ...
    let ca = decodedCookie.split('%3B'); // splits all cookies by ";"(%3B) so it has all cookies seperated
...
function toggleColorMode() { // function (gets called with button in header and toogles from light to dark)
    ...
    if (html.getAttribute('color-mode') === 'light') { // if current color mode is light
        ...
        document.cookie = "color-mode=dark;max-age:31536000"; // push to browser cookie                 illegal weil zu lange gespeichert aber für schule juckt ig
    } else {
        html.setAttribute('color-mode', 'light'); // set attribute dark
        document.cookie = "color-mode=light;max-age:31536000"; // push to browser cookie
    }
}
